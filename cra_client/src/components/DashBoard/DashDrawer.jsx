...
export default function DashDrawer() {
  const { user, userId, setCartOpen, scrollToTop } = useAppContext();
            ...
            >
              ...
              <div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl">
                ...
                <div className="relative mt-6 flex-1 px-4 sm:px-6">
                      ...
                      user.role === 'buyer' && (
                        <div className=''>
                          ...
                          <span className='cursor-pointer ml-4' onClick={() => setCartOpen(true)}>Cart</span>
										...
										<NavLink to={`/dashboard/${userId}/dashview`} className={'flex items-center gap-2 my-2'} onClick={handleNavClick}>
											<CgProfile className='transition-all duration-400 cursor-pointer hover:text-green-500' size={30} />
											<span>Profile</span>
										</NavLink>
										<NavLink to={`/dashboard/${userId}/notifications`} className={'flex items-center gap-2 my-2'} onClick={handleNavClick}>
                    ...
                    {
                      user.role === 'seller' && (
                        <NavLink to={`/dashboard/${userId}/adpost`} className={'flex items-center gap-2 my-2'} onClick={handleNavClick}>
                          ...
                          <span>New Ad</span>
										...
										<NavLink to={`/dashboard/${userId}/favorites`} className={'flex items-center gap-2 my-2'} onClick={handleNavClick}>
											<MdOutlineFavoriteBorder className='transition-all duration-400 cursor-pointer hover:text-green-500' size={30} />
											<span>Favorites</span>
										</NavLink>
										<NavLink to={`/dashboard/${userId}/settings`} className={'flex items-center gap-2 my-2'} onClick={handleNavClick}>
